...
int main(int argc, char** argv) {
    ...
    buffer* connect_message = create_message(1, 0, CONNECT, 0, 0, 0, 0);
	...
	for (;;) {
        if (connected == 0) {
            ...
            memcpy(h.data, buf, CST_HEADER_LEN);
            password = h.data[CST_IDENTIFIER];
            buffer* ack = create_message(1, password, CONNECT, 0, 0, 0, 0);
            ...
            struct sockaddr_storage from_addr;
            socklen_t from_addrlen = sizeof(from_addr);
            buffer* next_message = create_message(1, password, GPS, 0, 0, 0, 0);
            ssize_t bytes_sent = sendto(sock, connect_message->data, connect_message->len, 0, serv_addr->ai_addr, serv_addr->ai_addrlen);
            if (bytes_sent < 0) error("sendto()");
            ssize_t bytes_recv = recvfrom(sock, buf, BUFFER_LEN, 0, (struct sockaddr*)&from_addr, &from_addrlen);
            if (bytes_recv < 0) error("recvfrom()");
            header h;
            memcpy(h.data, buf, CST_HEADER_LEN);
            buf[h.data[CST_PAYLOAD_SIZE] + CST_HEADER_LEN] = '\0';
            fprintf(stdout, "%s\n", buf + h.data[CST_PAYLOAD_SIZE]);
            buffer* close_message = create_message(1, password, QUIT, 0, 0, 0, 0);
